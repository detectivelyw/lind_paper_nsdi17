...
be supported to run applications~\cite{tsai2016study}.  Recognizing this,
 \lip re-creates needed-but-risky OSVM functionality
...
SafePOSIX re-creates the broader POSIX functionalities needed by applications, while being contained within the Repy sandbox. An API in the sandbox only allows access to popular kernel paths, while
...
choosing popular kernel paths is a much more effective metric for identifying lines of code that are unlikely to contain flaws.
%of the kernel that
