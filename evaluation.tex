...
We then evaluated the protection afforded by four virtualization systems (including Lind) by attempting to trigger the 35 bugs from inside each.
...
\begin{tabular}{|p{1.7cm}|l|p{.6cm}|p{.6cm}|p{1cm}|p{.6cm}|}\hline

\multirow{2}{1.7cm}{\bf Vulnerability}    &  \multirow{2}{.7cm}{\bf Native Linux} 
...
\yiwen{My first try on the analysis, will work on this more.}
...
\brendan{How does the fuzzing work with Lind? I thought Trinity made system calls, rather than going through POSIX?}
...
and therefore accesses many risky lines of code. \brendan{Is there a better
example of complex kernel functionality that Graphene uses? It would be better
if it were something that Lind also supports.} By default, Docker does not wrap
...
1.4x to 2x when running applications such as Apache and UNIX benchmark \cappos{
What is this?}.
